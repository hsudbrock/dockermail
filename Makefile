...
run-dovecot:
	docker run -d -p 0.0.0.0:25:25 -p 0.0.0.0:587:587 -p 0.0.0.0:143:143 -v /srv/vmail:/srv/vmail -v /root/dockermail/config:/dockermail-config dovecot:2.1.7

run-rainloop:
	docker run -d -p 127.0.0.1:33100:80 -v /root/dockermail/config:/dockermail-config rainloop:1.6.9 

run-mailpile:
	docker run -d -p 127.0.0.1:33411:33411 mailpile:latest

run-owncloud:
	docker run -d -p 127.0.0.1:33200:80 -v /srv/owncloud:/var/www/owncloud/data -v /root/dockermail/config:/dockermail-config owncloud:8.0.2 
